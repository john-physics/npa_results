...
head('Staff Details',"$site | Staff Details","staff details"); //add header 
require 'menu.php';
...
$staffs = collect_table_data($conn,"staffs","surname ASC","*");
...
<table>
 ...
 <!-- <th>Subject(s) Handling</th> -->
   ...
   echo ' <tr>
     <td colspan="13" style="text-align:center;">No staff found.</td>
     </tr>';    
 ...
 $staff_email  = null_check($staff["email"],"None");
 $num = $staff["number"];
 $staff_num = null_check(std_num($num,"cc"),"None");
$title = $staff["title"];
  $sur = $staff["surname"];
  $oth = $staff["othernames"];
   $sur = ucwords(strtolower($sur));
  $oth = ucwords(strtolower($oth));
 $staff_name = $title." ".$sur." ".$oth;

 $lastlogin = null_check(timeAgo($staff["lastlogin"],'d/m/Y h:i A'),"Unknown");
 
  $acct_status = $staff["status"];
  $prf = $staff["profile"]??'null.jpg';
   $prf_link = '/images/staff/'.$prf;
...
$qulify = null_check($staff["qualifications"],'Unknown');
    ...
    $sn++;
     ...
     echo '<tr>
      <td>'.$sn.'.</td>
      <td>
      <img src="'.$prf_link.'" alt="staff photo" loading="lazy"  class="staff-photo" >
 ...
 <a href="/add_new?add_new_staff&action=edit-record&staff='.$staff_id.'&user_type=Staff" class="download-btn"><i class="fa-solid fa-pen-to-square"></i></a>
...
$totalStaff = count_user_data($conn,"staffs","status","Active","s");
...
echo '<div class="acct-container">
    ...
    <div class="info-item">
     <span class="label">Total Active Staff:</span>
      <span class="value"> '.number_format($totalStaff).' </span>
    </div>
